...
- `scripts/import_legacy_jsonl.py` преобразует унаследованный `Document`-JSONL
  в раздельные реестры и хранилище по SHA-256;
...
Для контролируемого переноса создан `scripts/import_legacy_jsonl.py`. Он
назначает `work_id` по правилу DOI → устойчивый ID источника → UUIDv5,
вычисляет `artifact_id=sha256:<хеш байтов>`, разносит PDF и извлечённый текст по
разным артефактам, явно фиксирует способ/масштаб получения и оставляет работу в
`pending`. По умолчанию команда работает
как предпросмотр. Она не превращает унаследованный файл в допущенный корпус и
не выводит права из содержимого `Document`.
