...
- `src/corpus/ocr_qa_distance.py` получает S/D/I, CER и WER непосредственно из
  файлов эталона и кандидата; остальные `src/corpus/ocr_qa_*.py` и
  `scripts/validate_ocr_qa.py` проверяют формы, файлы, связи, ревизии, сводные
  метрики и вложенный bootstrap одного запуска OCR QA;
...
- инженерный запуск `ufn_ocr_calibration_20260904_v2` зафиксировал 10 PDF и 29
  целевых страниц после замены двух страниц без русской прозы и исправления
  девяти печатных номеров; при подготовке 04.09.2026 созданы 29 PNG, 87
  кандидатов полной страницы, 87 пустых `candidate_prose` и 29 пустых
  заготовок ручного эталона;
- точные байты шести файлов реализации сохранены внутри запуска, а внешняя
  контрольная запись
  `manifests/seals/ufn_ocr_calibration_20260904_v2.json` фиксирует SHA-256
  паспорта;
- на 21.09.2026 вручную заполнены семь страниц из четырёх PDF; по девяти
  одинаковым областям прозы рассчитаны предварительные CER/WER трёх вариантов.
  Для выбранного `rus+eng` получены CER 0,25% и WER 1,56%; методика и все
  результаты приведены в [`UFN_OCR_PILOT.md`](UFN_OCR_PILOT.md);
- `pages.submitted.jsonl`, `formulas.submitted.jsonl` и формальная итоговая
  сводка ещё не созданы; критерии допуска по прозе, порядку чтения и формулам
  не проверены. Частичная инженерная калибровка не является корпусным
  качественным допуском;
...
4. вручную разметить подготовленный инженерный запуск
   `ufn_ocr_calibration_20260904_v2` и сформировать его результаты страниц,
   формул и сводку с CER/WER;
5. по итогам калибровки подготовить вероятностную выборку корпусного допуска из
   30–50 PDF, не менее 100 страниц и 20 000 вручную проверенных символов прозы,
   затем проверить CER/WER, порядок чтения и формулы валидатором;
