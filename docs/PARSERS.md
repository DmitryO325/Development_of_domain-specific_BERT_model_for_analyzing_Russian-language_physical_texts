...
| **Архив** | `draft/legacy.py`, `draft/import_legacy_jsonl.py` | неактивный прототип | прежний перенос `Document`-JSONL |
...
Прототип переноса прежнего `Document`-JSONL находится в `draft/legacy.py` и
`draft/import_legacy_jsonl.py`. Он не входит в активный конвейер и не
отслеживается Git. Старые локальные данные не переносим: корпус будет собран
заново по актуальным правилам. Для новых данных ещё нужно реализовать прямое
создание записей `works`, `artifacts` и `rights` без промежуточной миграции.
