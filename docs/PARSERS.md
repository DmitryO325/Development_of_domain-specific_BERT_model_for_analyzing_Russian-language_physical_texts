...
журналов уже создана, но реальные реестры ещё не заполнены. По умолчанию
сборщики сохраняют прототипный JSONL; с явным `--register-manifests` они
формируют `ManifestPlan` и вызывают `ManifestStore`. Пока они не передают
регистратору точный `HttpResponseSnapshot`, поэтому прямой режим создаёт
`metadata_only` и не допускает роль `full_text`. Построитель последующих
производных слоёв и версионированные разбиения также не реализованы.
...
`extract_best_text()` возвращает `(text, method, readable)`: текст, название
применённого метода и признак читаемости.
...
| `--fresh` | Очистить выходной файл перед первой успешной записью |
...
9. **Реестр и идентификаторы** — выбор `work_id` по правилу DOI → ID источника →
   ...
   архивного импортёра. Реальные реестры не заполнены; перед массовым запуском
   нужны действующие записи прав и передача исходных HTTP-снимков.
