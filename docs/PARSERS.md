...
Документ описывает **фактически реализованные модули сбора, извлечения текста
и первичного учёта корпуса**: какие материалы разбираются, как устроены URL,
какие поля попадают в JSONL и реестры, ограничения и отличия от блокнота Colab.
...
| **Реестры** | `src/corpus/` | данные корпуса | ID, JSON Schema, связи, хеши и неизменяемые байты |
| **Импорт** | `scripts/import_legacy_jsonl.py` | миграция | старый `Document`-JSONL → `works` + `artifacts` |
| **Аудит** | `scripts/validate_manifests.py` | проверка | схемы, ссылки, пути, размеры и SHA-256 |
...
Программная основа `artifact_id`, `work_id` и трёх реестров уже создана, но
реальные реестры ещё не заполнены, сборщики не подключены к ним напрямую,
построитель последующих производных слоёв и версионированные разбиения
отсутствуют.
...
extract_text_from_pdf()     # PyMuPDF page.get_text("text")
        │
        ├─ is_readable_russian? ──да──► метод "pdf", sidecar *_pdf.txt
        │
...
extract_text_from_pdf_ocr()  # Tesseract + учёт макета
        ...
        │
        ▼
"pdf_unreadable" + sidecar *_pdf_unreadable.txt
```
...
5. `extract_best_text` → `(text, local_path, readable, method)`; ошибка открытия
...
| Команда | Действие |
|---------|----------|
| `pilot` | 1 выпуск УФН (≤4 статьи) + RSS (8 записей) в `data/raw/pilot.jsonl` |
...
`pilot` не подменяет выходной путь и не удаляет файл без явного `--fresh`.

### 6.2. Импорт старого JSONL в реестры

Безопасный первый мост между прежними сборщиками и целевой архитектурой:
...
Без `--commit` это только предпросмотр: схемы, идентификаторы, локальные PDF и
хеши проверяются, но JSONL-реестры и слои `data/` не записываются. Параметр
`--allow-unresolved-rights` предназначен только для технической инвентаризации
до заполнения `rights.jsonl` и несовместим с `--commit`. Реальная запись также
отложена до принятия предложенного `DEC-013`. После выполнения этих условий:

- DOI имеет приоритет перед ID статьи УФН, затем применяется фиксированный
  UUIDv5;
- точные байты PDF и текста получают `artifact_id=sha256:<хеш>` и путь по хешу;
- PDF и извлечённый/OCR-текст становятся разными связанными артефактами;
- работа остаётся `pending`; импорт сам не назначает допуск в корпус;
- права передаются только через существующие `rights_record_id`, а не
  угадываются из текста статьи;
- фактические `acquisition_method` и `acquisition_scope` сохраняются в
  паспорте артефакта и точно сопоставляются с записью прав;
- неизвестный `text_source` не повышается до `full_text`; для старого
  импорта такая запись остаётся `metadata_only`;
- весь входной файл проверяется до первой записи, включая конфликты между его
  строками;
- SHA-256 входного JSONL повторно проверяется между проходами;
- точный повтор ничего не добавляет, несовпадающий повтор того же ID вызывает
  явный конфликт.

Старый сборщик не сохранял HTTP-статус, заголовки и точное время ответа.
Импортёр отмечает эту потерю происхождения предупреждением и оставляет
`retrievals` пустым; вымышленные время и хеш «полных» HTTP-метаданных не
создаются. Поэтому такой артефакт нельзя перевести в `eligible`, пока
требуемое доказательство не получено корректно. Рабочие реестры проверяются
командой:
...
Текущая реализация ещё не является полным механизмом жизненного цикла. Она не
сливает дополняющие друг друга карточки RSS и HTML, не обновляет
`pending → eligible`, не добавляет найденный позднее DOI и новое событие
получения к существующей записи. Например, пакетная проверка локального
`data/raw/corpus.jsonl` останавливается до записи: 20 строк соответствуют 16
работам, а четыре работы представлены двумя различающимися карточками. До
реального заполнения реестров нужно утвердить ревизии/события либо безопасное
атомарное обновление снимков `works` и `artifacts`.
...
8. **`pilot`** — это малый сетевой прогон, а не полная автономная проверка.
9. **Реестр и идентификаторы** — базовые стабильные `artifact_id`/`work_id`,
   схемы и хеши реализованы для импорта старых JSONL, но реальные реестры не
   заполнены, полные HTTP-метаданные старых загрузок восстановить нельзя, а
   модель ревизий текущего состояния ещё не утверждена.
...
11. **Тесты** — есть модульные регрессионные тесты HTTP-повторов, HTML,
   RSS/Atom, URL УФН, кэша PDF и CLI-`pilot`; эталонной выборки для оценки OCR пока нет.
...
# Проверить миграцию первых 10 записей без записи
python3 scripts/import_legacy_jsonl.py data/raw/ufn_100.jsonl \
  --profile ufn \
  --acquisition-method crawler \
  --acquisition-scope bulk \
...
`data/raw/pilot.jsonl`; его можно переопределить флагом `-o`. Каждый остальной пример
предполагает новый выходной путь. `--fresh` нельзя применять к
