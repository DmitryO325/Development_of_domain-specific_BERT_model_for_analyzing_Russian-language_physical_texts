...
| **PDF/OCR** | `src/collect/pdf_text.py` | извлечение текста | Скачивание PDF, прямое извлечение/OCR, явный выбор макета и постраничный экспорт |
| **RSS** | `src/collect/rss_feed.py` | лента | WordPress/RSS/Atom |
| **API ОИЯИ** | `src/collect/jinr_api.py`, `scripts/download_jinr.py` | клиент API | Минутные паузы, точные ответы и HTTP-свидетельства, страницы метаданных и PDF по UUID; 21.09 завершены и проверены 54 страницы / 5 380 уникальных записей |
...
Клиент ОИЯИ пока сохраняет отдельный исходный слой, а не регистрирует работы
или допуск к обучению. Команды, ограничения кеша и состояние соединения описаны
в [`JINR_API.md`](JINR_API.md).
...
| `pdf_text_file` | сопроводительный файл | `.../pdf-text-rus-eng-v2/r265b_pdf_ocr_layout.txt` для нового OCR |
...
Это **не модуль разбора HTML**, а подсистема для бинарных PDF. Для УФН
сохранён прежний алгоритм, для других источников можно явно выбрать
одноколоночный или двухколоночный макет OCR.
...
| **pytesseract** + **Tesseract** | OCR; с 21.09.2026 в коде по умолчанию `rus+eng` |
...
Параметр `ocr_layout` управляет только резервной веткой OCR: читаемый
встроенный слой по-прежнему используется первым. Выбор макета не включает
принудительное распознавание, а `try_ocr=False` полностью отключает OCR.
...
### 4.4. Явный выбор макета OCR

С 23.09.2026 доступен параметр `ocr_layout` с тремя значениями:

- `ufn` — режим по умолчанию: на первой странице шапка на всю ширину,
  затем две колонки; со второй страницы сразу две колонки. Сохранена
  прежняя эвристика УФН, описанная ниже.
- `single-column` — каждая страница распознаётся целиком, без разрезания
  строк посередине.
- `two-column` — на каждой странице сначала распознаётся левая половина,
  затем правая, без отдельного выделения шапки.

Универсального автоматического определения макета нет. Режим выбирают по
реальному PDF; для документов со смешанными макетами нужна отдельная
проверка. `scrape.py ufn` по-прежнему использует `ufn`.

**Проблема:** Tesseract по умолчанию читает строки слева направо через всю ширину → текст левой и правой колонки перемешивается.

**Прежний алгоритм в режиме `ufn`:**
...
**Текущие параметры по умолчанию в коде:** DPI **200**, Tesseract `--psm 6`
(блок текста), язык `rus+eng`.

**Выбранный рабочий вариант с 21.09.2026:** Tesseract `rus+eng`. На семи
страницах пилота он дал CER **0,25%** и WER **1,56%**, против **1,25% / 2,76%**
у `rus` и **82,59% / 96,00%** у прямого текстового слоя PDF. Сравнение OCR
выполнено с Tesseract 5.5.3, DPI 200 и `--psm 6`. С разрешения пользователя
язык по умолчанию переключён в Python, версия прогонов в режиме `ufn` —
`pdf-text-rus-eng-v2`. Прежние артефакты и зафиксированные варианты калибровки
не изменялись. Подробности и ограничения — в разделе 6
[`UFN_OCR_PILOT.md`](UFN_OCR_PILOT.md).

Функция `pdf_extraction_version()` определяет стандартную версию по макету:
для `single-column` — `pdf-text-rus-eng-single-column-v3`, для `two-column` —
`pdf-text-rus-eng-two-column-v3`. Версия `ufn` остаётся прежней. Язык, DPI и
PSM при выборе нового макета не меняются.
...
Новый OCR сохраняется отдельно от прежних результатов:
`data/raw/pdf_text/{extraction_version}/{stem}_pdf_ocr_layout.txt`, например
в `pdf-text-rus-eng-v2/` для `ufn` или
`pdf-text-rus-eng-single-column-v3/` для `single-column`.
При пользовательском `--pdf-text-dir` меняется корневой каталог, но сохраняется
подкаталог версии. Для прямого текстового слоя и метода `pdf_unreadable` путь
прежний: `data/raw/pdf_text/{stem}_{method}.txt`.
...
Заголовок файла: метод, имя PDF, число символов; для OCR также указаны язык,
фактически выбранный макет и версия извлечения.
...
| `--ocr-layout` | `ufn` (по умолчанию), `single-column` или `two-column`; влияет только на OCR |
| `--extraction-version` | Указать собственную версию метода и каталог производного слоя; по умолчанию версия определяется по `--ocr-layout` |
...
Для одноколоночных PDF выбирают `--ocr-layout single-column`; стандартная
версия будет `pdf-text-rus-eng-single-column-v3`. Для двухколоночных PDF без
общей шапки — `--ocr-layout two-column` и
`pdf-text-rus-eng-two-column-v3`. В режиме `ufn` сохраняется
`pdf-text-rus-eng-v2`. Явная собственная версия разрешена, но стандартную
версию другого макета указать нельзя: такая комбинация отклоняется до
обработки. Выбранный `ocr_layout` записывается в локальный отчёт.

Флаг `--no-ocr` отключает распознавание независимо от выбранного макета.
Даже без этого флага читаемый встроенный текст не заменяется результатом OCR.

Отчёт по умолчанию сохраняется в
`manifests/results/<имя_входного_файла_без_расширения>_<extraction_version>_extraction.jsonl`.
Например, для пилота УФН с новой версией это
`ufn_manual_20260831_pdf-text-rus-eng-v2_extraction.jsonl`; прежний отчёт
31 августа не заменяется. Явный `--report` по-прежнему задаёт точный путь.
...
**Ограничение повторной регистрации:** если другая версия извлечения даёт
в точности те же байты текста, текущий реестр отклоняет регистрацию из-за
различия происхождения одного и того же артефакта (`ManifestConflictError`).
Старый артефакт при этом не перезаписывается. Перед массовой пересборкой
ранее зарегистрированных PDF нужно отдельно решить, как учитывать такие
совпадающие результаты разных версий.
...
изменений реестров. Эта команда экспортирует один автоматически выбранный
результат. Параллельные варианты калибровочного запуска формирует отдельная
команда `prepare_ocr_qa.py`.

### 7.2. Подготовка инженерной калибровки OCR

4 сентября 2026 года команда подготовила локальный запуск
`ufn_ocr_calibration_20260904_v2`:
...
Зафиксированы 10 PDF и 29 целенаправленно выбранных страниц. Финальная
пересборка заменила две страницы без русской прозы и исправила девять печатных
номеров. Для каждой страницы создан PNG и три текстовых кандидата: прямой
текстовый слой PDF, OCR `rus` и OCR `rus+eng`. Всего сформированы 29 PNG, 87
кандидатов полной страницы, 87 пустых заготовок `candidate_prose` и 29 пустых
заготовок ручного эталона. Точные байты шести файлов реализации сохранены
внутри запуска, а внешняя контрольная запись
`manifests/seals/ufn_ocr_calibration_20260904_v2.json`
фиксирует SHA-256 паспорта. Это инженерная калибровка, а не вероятностная
выборка для корпусного качественного допуска.

К 21 сентября 2026 года вручную размечены семь страниц четырёх PDF и
рассчитаны предварительные CER/WER по девяти областям прозы: 19 339 символов
и 2 498 слов после нормализации. Показатели трёх вариантов приведены в
разделе 4.4; подробные локальные результаты находятся в
`manifests/results/ocr_seven_20260921/metrics.json` (вне Git). Ручной выбор
областей не проверяет весь конвейер выделения текста и порядок чтения страницы.

### 7.3. Машинная проверка OCR QA
...
SHA-256 файлов, интервалы и области. Через `src/corpus/ocr_qa_distance.py`
валидатор заново получает S/D/I, CER и WER непосредственно из текстовых файлов,
пересчитывает F1 и доли ошибок, а также воспроизводит вложенный bootstrap по
работам и страницам.
...
реальный запуск. У калибровочного запуска уже есть паспорт, кадр, изображения,
кандидаты и частичный ручной эталон с предварительными CER/WER по семи
страницам. Формальные `pages.submitted.jsonl`, `formulas.submitted.jsonl` и
`ocr_qa_summary.json` ещё не созданы; 29 страниц не завершены и критерий
качества OCR не пройден. Выбор `rus+eng` не присваивает артефактам `approved`.
...
| Постраничный экспорт и проверка OCR QA | **нет** | да; предварительные CER/WER по 7 страницам, формальный допуск не пройден |
...
3. **Макет OCR** — выбирается явно, автоматического определения нет.
   Эвристика границы шапки и двух колонок действует только в режиме `ufn`
   и может ошибаться; смешанная вёрстка требует отдельной проверки.
...
11. **Тесты** — есть модульные регрессионные тесты HTTP-повторов, HTML,
   ...
   экспорта, подготовки калибровки, связей OCR QA и пересчёта метрик. Частичная
   разметка семи страниц не завершает ручной эталон всего запуска.
12. **Критерии допуска** — предварительные CER/WER измерены на семи
   целенаправленно выбранных страницах, но формальный критерий качества ещё
   не проверен. До допуска к экспериментам должны быть выполнены критерии
   из плана для односторонних 95%-ных кластерных доверительных интервалов:
   верхние границы CER ≤ 3%, WER ≤ 10% и доли критических ошибок порядка
   чтения ≤ 5%; для H3 нижняя граница F1 обнаружения формул ≥ 0.90, а верхняя
   граница критической порчи формул ≤ 5%.
