...
class SourceProfile:
    ...
    def native_id(self, url: str, extra: dict) -> str | None:
        ...
        if self.key == "ufn":
            match = re.search(r"/ru/articles/(\d{4})/(\d+)/([a-z])(?:/|$)", url, re.I)
            if match:
...
def get_source_profile(name: str, *, source: str = "", url: str = "") -> SourceProfile:
    ...
    if name != "auto":
        try:
            ...
            raise ValueError(f"Неизвестный профиль {name!r}; доступны: {choices}, auto") from exc
    ...
    matches = [profile for profile in SOURCE_PROFILES.values() if profile.matches(source, url)]
    if len(matches) != 1:
        raise ValueError(
